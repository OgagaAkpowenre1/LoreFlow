...
function SequenceEditor({ nodeId, lines = [] }) {
  ...
  // ── REFACTORED VARIANT & COMPOUND LOGIC HANDLERS ──
  ...
  // ── NEW CONDITION HANDLERS FOR VARIANTS ──

  const addConditionToVariant = useCallback(
    (lineIndex, variantIndex) => {
      const newLines = [...getCurrentLines()];
      const targetLine = { ...newLines[lineIndex] };
      const currentVariants = [...targetLine.variants];
      ...
      // Ensure conditions array exists (migrates legacy on-the-fly)
      ...
      currentVariants[variantIndex] = variant;
      targetLine.variants = currentVariants;
      newLines[lineIndex] = targetLine;
      syncDialogueLinesWithStore(newLines);
    },
  ...
  const updateConditionInVariant = useCallback(
    (lineIndex, variantIndex, conditionIndex, fieldId, value) => {
      ...
      // Auto-migrate legacy variant format to conditions array
      let conditions = [...(variant.conditions || [])];
      if (conditions.length === 0 && variant.check_flag) {
        conditions.push({
          id: crypto.randomUUID(),
          check_flag: variant.check_flag,
          operator: variant.operator || "==",
        ...
        // Wipe legacy root fields to prevent duplication bugs
        delete variant.check_flag;
  ...
  const removeConditionFromVariant = useCallback(
              ...
              <div className="flex-grow overflow-y-auto p-6 space-y-6 custom-scrollbar bg-white">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    >
                      <Plus size={12} /> Add Variant Scenario
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {currentVariants.map((variant, vIdx) => {
                      // Dynamically fallback for backwards compatibility
                      const isLegacy =
                        !variant.conditions && variant.check_flag;
                      const conditions =
                        variant.conditions ||
                        (isLegacy
                          ? [
                      ...
                      return (
                        <div
                          key={variant.id || vIdx}
                          className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-4 relative animate-in fade-in zoom-in-95 duration-150"
                        >
                          {/* Top Action Row & Logic Operator Toggle */}
                          <div className="flex items-center justify-between pb-2 border-b border-gray-200">
                            <div className="flex items-center gap-2">
                              ...
                              <select
                                value={variant.logicalOperator || "AND"}
                                onChange={(e) =>
                                  updateVariant(
                                    activeIndex,
                                    vIdx,
                                ...
                                className="text-xs font-black bg-white border border-gray-200 text-blue-600 rounded px-2 py-1 outline-none cursor-pointer hover:border-blue-300"
                              >
                          ...
                          <div className="space-y-2 pl-3 border-l-2 border-blue-200">
                            {conditions.map((cond, cIdx) => {
                              ...
                              return (
                                <div
                                  key={cond.id || cIdx}
                                  className="flex items-center gap-2 bg-white border border-gray-100 rounded-lg p-2 shadow-sm"
                                >
                                  <span className="font-black text-blue-500 text-[10px] uppercase flex-shrink-0 w-4 text-center">
                                    {cIdx === 0
                                      ? "IF"
                            ...
                            <button
                              onClick={() =>
                                addConditionToVariant(activeIndex, vIdx)
                              }
                              className="text-[10px] text-blue-600 hover:text-blue-800 font-bold uppercase tracking-wider flex items-center gap-1 mt-2"
                            >
                          ...
                          {/* ── OVERRIDE PROPS (Speaker, Portrait, etc.) ── */}
                          <div className="space-y-3">
                            <label className="text-[9px] font-black text-gray-400 uppercase tracking-tight flex justify-between">
                              <span>Property Overrides (Optional)</span>
                              <span className="font-normal italic">
                                Leave empty to inherit from parent line
                              </span>
                            </label>

                            <div className="grid grid-cols-2 gap-3">
                              {schema.sequenceFields
                                .filter((f) => f.id !== "text")
                                .map((field) => (
                          ...
                          <div className="space-y-1 pt-2">
                            ...
                            <textarea
                              ...
                              placeholder='e.g., "Welcome back, [player_name]!"'
                          ...
                          {/* ── INLINE VARIANT SPECIFIC TOKEN VERIFICATION ── */}
