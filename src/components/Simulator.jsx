...
import React, { useState, useEffect, useRef, memo } from "react";
import { useLoreStore } from "../store";
import { evaluateConditionGroup, resolveDialogueLine } from "../lib/dialogueLogic";
import {
  X,
  Play,
  RefreshCw,
...
function Simulator() {
  ...
  const stepToNode = (nodeId, graphName) => {
    ...
    if (node.type === "logic") {
      ...
      const logicalOperator = node.data?.logicalOperator || "AND";
      const isTrue = evaluateConditionGroup(
        logicalOperator,
        conditions,
        evaluateCondition,
      ...
      if (conditions.length > 0) {
        const summary = conditions
          .map((c) => `${c.check_flag} ${c.operator} ${c.value}`)
          .join(` ${logicalOperator} `);
        addLog(`Condition (${summary}) evaluated to ${isTrue}`, "system");
      }
      const nextId = getNextNodeId(
        node.id,
        graphName,
            ...
            <div className="flex-grow p-6 overflow-y-auto custom-scrollbar">
              {!currentNode ? (
                ...
                <div className="w-full max-w-2xl mx-auto space-y-4 animate-in fade-in slide-in-from-bottom-4 pb-12">
                  ...
                  <div className="bg-white border-2 border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
                    {currentNode.data?.dialogueLines?.map((line, i) => {
                      // Resolves the first matching variant (compound
                      // AND/OR conditions) and merges its overridden
                      // fields (speaker/text/portrait/etc.) on top of the
                      // base line - any override left blank falls back to
                      // the base line's own value.
                      const { line: resolvedLine } = resolveDialogueLine(
                        line,
                        evaluateCondition,
                        (name) => sandboxState[name],
                      );
