...
- **Logic-Embedded Edges:** Connections aren't just lines; they hold conditional data (e.g., _"Only show this choice if `gold > 50`"_).
...
_Goal: Prove the canvas works._
...
_Goal: Make it look like a writing tool, not a generic flowchart._
...
_Goal: Make the nodes editable without having to type directly on the tiny canvas boxes._
...
_Goal: Make it a real developer tool._
...
You don't want to write a unique `input` for every field. You want a component that says: _"I see a field of type 'list', so I will render a Dropdown. Oh, and its source is 'npc_names'."_
...
Building a **Schema-Driven System** takes more time _at the start_ (about 2 extra days), but it makes the rest of the project **infinitely faster**.
